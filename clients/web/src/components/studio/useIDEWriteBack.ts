...
// back into projectFiles so Studio + the finisher loop see edits the
// operator made inside the cloud IDE.
//
// Flow:
//   1. Studio projectFiles → /api/ide/sync POST (handled in
...
//      refetch ProjectFiles so the Studio data model updates.
//
...
export interface UseIDEWriteBackArgs {
  projectID: string;
  // Wait until the iframe has loaded AND the initial Studio→IDE push
  // finished. Otherwise we'd push the empty pre-seed state straight
  // back over projectFiles.
  enabled: boolean;
  // The current Studio snapshot used to prime the "last pushed"
