...
export function AutomationsPane() {
  ...
  const remove = (a: Automation) => void mutate('Deleted', (req) => req('DeleteAutomation', operations.DELETE_AUTOMATION, { id: a.id }), invalidate);
  ...
  const columns = useMemo<DataGridColumn<Automation>[]>(() => [
    { field: 'name', headerName: 'Automation', flex: 1, minWidth: 200, cellRenderer: ({ data }: DataGridCellParams<Automation>) => data ? (
      <Stack direction="row" alignItems="center" spacing={1}>
        <Box sx={{ fontSize: '1rem' }}>{triggerIcon(data.triggerKind)}</Box>
        <Stack spacing={0} sx={{ lineHeight: 1.2, py: 0.5 }}>
          <Typography sx={{ fontSize: '0.84rem' }} noWrap>{data.name}</Typography>
          <Typography sx={(th) => ({ fontFamily: th.brand.font.mono, fontSize: '0.66rem', color: 'text.secondary' })} noWrap>{data.triggerKind} · {data.triggerConfig}</Typography>
        </Stack>
      </Stack>
    ) : null },
    { field: 'action', headerName: 'Action', flex: 1, minWidth: 160, cellRenderer: ({ value }: DataGridCellParams<Automation, string>) => <Typography sx={(th) => ({ fontFamily: th.brand.font.mono, fontSize: '0.74rem', color: 'text.secondary' })} noWrap>{value}</Typography> },
    { field: 'lastStatus', headerName: 'Last run', width: 140, cellRenderer: ({ data }: DataGridCellParams<Automation>) => data ? (
      <Stack direction="row" alignItems="center" spacing={0.75}>
        ...
        <Typography sx={{ fontSize: '0.78rem', color: 'text.secondary' }}>{data.runs} runs</Typography>
      </Stack>
    ) : null },
    ...
    { colId: 'actions', headerName: '', width: 150, sortable: false, filter: false, cellRenderer: ({ data }: DataGridCellParams<Automation>) => data ? (
      <Stack direction="row" spacing={0.5}>
        ...
        <Button size="small" variant="text" color="inherit" disabled={busy} onClick={(e) => { e.stopPropagation(); remove(data); }}>Delete</Button>
      </Stack>
    ) : null },
  ], [t, busy]);
  ...
  return (
    <Box sx={{ flex: 1, height: '100%', overflowY: 'auto', bgcolor: 'background.default', p: 3 }}>
      <Box sx={{ maxWidth: 1080, mx: 'auto' }}>
        ...
        <Card sx={{ p: 2.5, mb: 3 }}>
          <Typography sx={(th) => ({ fontFamily: th.brand.font.mono, fontSize: '0.66rem', textTransform: 'uppercase', color: 'text.disabled', mb: 1.5 })}>New automation</Typography>
