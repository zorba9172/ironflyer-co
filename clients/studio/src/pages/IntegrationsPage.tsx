...
interface GateVerdict { gate: string; status: string }
            ...
            <Card key={c.name} sx={{ p: 2.5 }}>
              <Stack direction="row" alignItems="center" spacing={1.5} sx={{ mb: 1.5 }}>
                ...
                <Box sx={{ minWidth: 0, flex: 1 }}>
                  ...
                  <Typography sx={(t) => ({ fontFamily: t.brand.font.mono, fontSize: '0.68rem', color: 'text.disabled' })}>{c.label} · {c.gate ?? 'no gate yet'}</Typography>
                </Box>
                {status && (
                  <Chip size="small" label={status} color={passed ? 'success' : 'warning'} variant="outlined" sx={{ height: 20, fontSize: '0.62rem' }} />
                )}
              </Stack>
