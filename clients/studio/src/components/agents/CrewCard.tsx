  ...
  return (
    <Card sx={{ p: 2.5, display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
        <Typography variant="h6" sx={{ fontSize: text.s102 }} noWrap>{crew.name || 'Untitled crew'}</Typography>
        <Chip size="small" label={crewProcessLabel(crew.process)} sx={(t) => ({ height: 20, fontSize: text.s66, fontFamily: t.brand.font.mono, bgcolor: `${t.palette.primary.main}1f`, color: 'primary.main' })} />
      </Stack>
      <Typography sx={{ color: 'text.secondary', fontSize: text.s86, mb: 1.25 }}>{crew.goal || 'No goal set'}</Typography>
      ...
      <Stack direction="row" alignItems="center" sx={{ pl: '6px', mb: 1.5 }}>
        ...
        {memberNames.length > 6 && <Typography sx={{ fontSize: text.s72, color: 'text.disabled', ml: 1 }}>+{memberNames.length - 6}</Typography>}
        {memberNames.length === 0 && <Typography sx={{ fontSize: text.s78, color: 'text.disabled' }}>No members</Typography>}
      </Stack>
      ...
      <Box sx={{ flex: 1 }} />
      <Typography sx={(t) => ({ fontFamily: t.brand.font.mono, fontSize: text.s66, color: 'text.disabled', mt: 0.5 })}>
