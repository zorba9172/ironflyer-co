...
|---|---|---|---|---|
| W1 | `walletCreateTopUp` returns `wallet_topper: not configured` in prod. **Real users cannot add money to their wallet, which means no paid execution can ever start in prod.** This is the only thing standing between the live stack and a usable Base44-class experience. | **P0** | `wallet.Topper` is keyed off `STRIPE_SECRET_KEY` and prod has it empty. The Paddle + Lemon Squeezy keys the operator filled into `.env.production.local` are wired into the `budget` Stripe-checkout endpoint, but **not** into `wallet.Topper` — that adapter doesn't exist yet. Adding it is feature work the operator explicitly took off the table this round ("לא לפתח דברים חדשים"). | **Pick one:** (a) issue a Stripe restricted key (charges:write + customers:write + checkout_sessions:write + subscriptions:write + webhooks:write), put it in `infra/compose/.env.prod` as `STRIPE_SECRET_KEY=…` + `STRIPE_WEBHOOK_SECRET=…`, redeploy the orchestrator; (b) seed dev wallets via `IRONFLYER_DEV_WALLET_SEED_USD>0` on the orchestrator env (only safe for closed pilots — every new user gets a free balance). |
