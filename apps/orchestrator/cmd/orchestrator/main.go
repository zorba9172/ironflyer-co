...
import (
	...
	"ironflyer/apps/orchestrator/internal/dashboards/adapters"
	...
	"ironflyer/apps/orchestrator/internal/wireup"
	"ironflyer/apps/orchestrator/migrations"
)
...
func main() {
	...
	if chRes.Client != nil {
		...
		go func() {
			if err := correction.Run(ctx); err != nil && !errors.Is(err, context.Canceled) {
	...
	if deploySvc != nil {
		...
		go func() {
			if err := deploySweeper.Run(ctx); err != nil && !errors.Is(err, context.Canceled) {
