// TODO(mobile-runtime): swap to GraphQL useQuery once apps/runtime mobile HTTP routes are exposed via the orchestrator schema. For now this is mock data shaped like the real response.
...
// apps/runtime/internal/mobile/metro.go) so the cockpit can render the
