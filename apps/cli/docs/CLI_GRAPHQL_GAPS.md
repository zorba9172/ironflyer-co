...
The CLI's transport layer is now genqlient-powered (`apps/cli/internal/gql`).
...
- **`Project.gates`** — `domain.Project` carries a gate-state map but
  `apps/orchestrator/internal/graph/schema/projects.graphql` does not
...
`apps/cli/internal/commands/{run,deploy}.go` did not have to change.
...
`apps/runtime`) adapted to genqlient's `Dialer` / `WSConn` interfaces.
