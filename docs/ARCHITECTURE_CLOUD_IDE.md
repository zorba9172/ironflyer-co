...
- Primary base: `code-server`, because the runtime already builds `infra/docker/ironflyer-code.Dockerfile`, the Docker driver returns `ideUrl`, and local compose exposes a branded code-server service.
- Candidate fallback: OpenVSCode Server, if we need closer upstream VS Code behavior with less self-hosted customization.
...
- No public unauthenticated IDE URLs.
