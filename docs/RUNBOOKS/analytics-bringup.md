...
2. **DLQ topic auto-create.** The first publisher attempt against a
   fresh Redpanda logs
   `events publisher: DLQ publish failed; dead row retained ...
   Unknown Topic Or Partition` for
   `ifly.dev.dlq.billing.ledger.outbox_publisher.v1`. The retried
   publish to the original topic succeeds on the next loop, so events
   do reach CH — but the DLQ path itself never primes its topic. Fix:
   pre-create the DLQ topics at startup the same way the publisher
   eagerly creates the source topics.

3. **Schema Registry subjects for the publisher path.** The publisher
   warns `outboxhooks: schema subject not registered; inserting
   without validation` for every wallet/ledger event. The 9 V22
   schemas register at orchestrator boot (against the in-process
   registry) but the outbox-hook lookup uses the
   `<topic>-<event_type>` subject form which is never registered —
   only the `<topic>-default` subject is. Either widen the hook
   lookup to fall back to `-default` or register per-event subjects.

4. **Execution lifecycle events.** `createPaidExecution` admits the
